...
const translateErr = (err) =>
  err === ERR_CANNOT_CONNECT
    ...
    ? "Please enter a RiverOS URL."
    : err === ERR_INVALID_HTTPS_TO_HTTP
    ? html`
        Cannot connect to RiverOS instances over "http://".
        ${seeFAQ("https")}
      `
    : `Unknown error (${err}).`;
...
const INTRO = html`
  <p>
    RiverOS Cast allows you to cast your RiverOS installation to
...
export class HcConnect extends LitElement {
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            ${INTRO}
            <p>
              To get started, enter your RiverOS URL and click authorize.
  ...
  private async _handleConnect() {
    ...
    if (value === "") {
      this.error = "Please enter a RiverOS URL.";
      return;
    }
    if (value.indexOf("://") === -1) {
      this.error =
